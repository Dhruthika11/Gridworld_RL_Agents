...
function getMaxSteps(gridSize) {
    return Math.pow(gridSize, 3) + 200; 
}
...
function startTraining() {
    ...
    const episodesPerStep = episodesInput ? parseInt(episodesInput.value) : 200;
...
function updateDisplay() {
    ...
    // --- 🚀 CONVERGENCE CHECKER ---
    // If average reward > 900, we consider it "Converged"
    if (qVal > 900 && q_converged_episode === null) {
        q_converged_episode = total_episodes;
    }
    if (sarsaVal > 900 && sarsa_converged_episode === null) {
        sarsa_converged_episode = total_episodes;
    }

    // Update Convergence UI
    ...
    const getRecentSuccess = (arr) => {
        if (arr.length === 0) return '0.0';
        const wins = arr.filter(r => r > 0).length; 
        return ((wins / arr.length) * 100).toFixed(1);
    };

    const qRate = getRecentSuccess(q_rewards);
    const sarsaRate = getRecentSuccess(sarsa_rewards);
    const naiveRate = getRecentSuccess(naive_rewards);

    document.getElementById('success-rate').textContent = `Q: ${qRate}% | SARSA: ${sarsaRate}% | Naive: ${naiveRate}%`;
...
function changeGridSize(newSize) {
    ...
    if(document.getElementById('exploration-decay')) {
        document.getElementById('exploration-decay').value = EXPLORATION_DECAY_RATE;
        document.getElementById('exploration-decay-value').textContent = EXPLORATION_DECAY_RATE;
    }
...
function resetAgent() {
    ...
    q_converged_episode = null; // Reset Convergence
    sarsa_converged_episode = null; // Reset Convergence
    ...
    if (winRateChart) {
        ...
        winRateChart.data.datasets.forEach(d => d.data = []);
        winRateChart.update();
    }
...
function runSelectedPolicy() {
    ...
    const step_fn = () => {
        ...
        // Add animated arrow to current cell
...
function setupCharts() {
    ...
    winRateChart = new Chart(ctxW, {
        ...
        options: { ...common, scales: { ...common.scales, y: { max: 100, min: 0, grid: { color: 'rgba(148, 163, 184, 0.1)' } } } }
    });
}
...
function updateCharts() {
    ...
    winRateChart.data.labels = rewardChart.data.labels;
    ...
    const dsW = winRateChart.data.datasets;
    dsW[0].data.push(getWinRate(q_rewards));
    dsW[1].data.push(getWinRate(sarsa_rewards));
    dsW[2].data.push(getWinRate(naive_rewards));
    if (dsW[0].data.length > 100) dsW.forEach(d => d.data.shift());
